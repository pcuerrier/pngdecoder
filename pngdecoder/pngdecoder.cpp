...
static uint32_t crc32_fast(const uint32_t crc, const uint8_t* memory, size_t n)
{
    uint32_t result = crc;
    ...
    {
        uint8_t ch = memory[i];
        uint32_t t = (ch^result) & 0xFF;
        result = (result >> 8) ^ crc32_table[t];
    }
...
PngResult decodePng(const char* filename)
{
    ...
    while (fileContent.fileSize > 0)
    {
        ...
        if (chunkHeader)
        {
            ...
            void* chunkData = readSize(&fileContent, chunkHeader->length);
            ...
            // CRC-checking for each chunks
            {
                uint32_t crcType = crc32_fast(0xFFFFFFFF, reinterpret_cast<uint8_t*>(&chunkHeader->typeU32), 4);
                uint32_t crcChunk = crc32_fast(~crcType, reinterpret_cast<uint8_t*>(chunkData), chunkHeader->length);

                if (crcChunk != chunkFooter->crc)
                {
                    result.errorCode = PNG_INVALID_CHECKSUM;
                    printf("%u - %u - %u\n", chunkFooter->crc, crcType, crcChunk);
                    return result;
                }
            }
            ...
            else if (chunkHeader->typeU32 == (int)'gAMA')
            {
                PngGAMA* gama = reinterpret_cast<PngGAMA*>( chunkData );
                printf("Gamma: %u\n", gama->gamma);
            }
            else if (chunkHeader->typeU32 == (int)'IDAT')
            {
                printf("DATA\n");
            }
            else if (chunkHeader->typeU32 == (int)'IEND')
            {
                if (fileContent.fileSize != 0)
                {
                    result.errorCode = PNG_INVALID_CHUNK_ORDER;
                    return result;
                }
            }
        }
