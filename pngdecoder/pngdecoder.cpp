...
PngResult decodePng(const char* filename)
{
    ...
    while (fileContent.fileSize > 0)
    {
        ...
        if (chunkHeader)
        {
            ...
            //uint8_t* crcChunk = reinterpret_cast<uint8_t*>(chunkHeader) + sizeof(length);
            //int crcChunkLen = length + sizeof(type);

            void* chunkData = readSize(&fileContent, chunkHeader->length);
            /*PngChunkFooter* chunkFooter = */Read(&fileContent, PngChunkFooter);
            //uint32_t crc = endianSwap(chunkFooter->crc);

            printf("%c%c%c%c\n", chunkHeader->type[3], chunkHeader->type[2],
                                 chunkHeader->type[1], chunkHeader->type[0]);
            if (chunkHeader->typeU32 == (int)'IHDR')
            {
                ...
                /*if (chunkFooter->crc != crc32(0xFFFFFFFF, crcChunk, crcChunkLen))
                {
